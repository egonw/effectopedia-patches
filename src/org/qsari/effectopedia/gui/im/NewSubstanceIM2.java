...
public class NewSubstanceIM2 extends IndexedGOIM
	{
		public NewSubstanceIM2(PathwaysViewUI pathwayUI, InterfaceModes owner)
			{
		...
		protected void updateTempPE(int x, int y)
			{
				...
				if ((go != null) && ((go.getO() == tempPE)))
					return;
				removeTempPE();
				tempPE = currentElement.getCurrent();// currentElement.whatWillBe(0, 0);
				pathwaysView.update();
				addTempPE(x, y);
				pathwaysView.update();
			}
