		...
		public static class XTableColumn extends TableColumn
			{
				 ...
				 * TableColumnModels, there may be times where a column spans more columns
				 * than there are at the right of it. This state is only allowed during such
				 * modifications. Once they are finished, all spans must not be too large.
				 * XTableColumns that are hidden by the spans of their predecessors are
				 * ignored (in the TableHeader, of course not by the JTable).
				 * 
				 ...
				 * event firePropertyChange("width", getWidth(), getWidth()); which would
				 * handle that (only width and preferred width changes can reach the
				 * header). The moral is: don't change spans later, or if, repaint the
				 * header manually.
				 */
