 ...
 */
public class DataSourceEntriesTableUI extends ContextSensitivePanel implements ListSelectionListener, RowSorterListener
	{
		/**
	 * 
	 */
		...
		public static void main(String[] args)
			{
				...
				frame.getContentPane().add(new DataSourceEntriesTableUI(null));
				frame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
				frame.pack();
				frame.setVisible(true);
			}
		
		public DataSourceEntriesTableUI(RootHelpContext rootHelpContext)
			{
				super(rootHelpContext);
				setName("data_source_entries");
		...
		public class LiveEntriesTableModel extends DefaultTableModel
			{
				...
				public int getRowCount()
					{
						if (data != null)
							return data.getLivePathwayElements().getMap().size();
						else
							return 0;
					}
				...
				public Object getValueAt(int row, int column)
					{
						SourceTrace st = null;
						if (row < pathwaysCount)
							st = pathways.get(row);
						else if (row < pathwaysCount + fragmentsCount)
							st = fragments.get(row - pathwaysCount);
						if (st != null)
							{
								if (column == 0)
									return UIResources.getIconForClass(TraceableClasses.REGISTERED.getClassByID(st.getClassID()));
								if (column == 1)
									{
										...
										if (eo == null)
											{
												if (eo instanceof PathwayElement)
													return ((PathwayElement) eo).getTitle();
												else if (eo instanceof Pathway)
													return ((Pathway) eo).getTitle();
											}
										return null;
									}
							}
		 ...
		 */
				...
				public ObjectUnionTableModel(boolean readonly)
					{
						super();
				...
				public int getRowCount()
					{
						if (list != null)
							return list.getList(sortIndex).size();
						else
							return 0;
					}
				...
				public Object getValueAt(int row, int column)
					{
						...
						if (eou != null)
							{
								if (column == 0)
									return UIResources.getIconForClass(sourceA ? eou.getObjectClassA() : eou.getObjectClassB());
								if (column == 1)
									{
												...
												if (eo instanceof PathwayElement)
													{
														if ((((PathwayElement) eo).getTitle().equals("")) && (eo instanceof Describable))
															return ((Describable) eo).getGenericDescription();
														else
															return ((PathwayElement) eo).getTitle();
													}
												else if (eo instanceof Pathway)
						...
						if (list != null)
							{
								...
								if (sychronizedTable != null)
									sychronizedTable.sychronizeRows(row);
							}
					}
				else
					{
		...
		public Object getSelectedRawObject()
			{
				...
				if (row == -1)
					return null;
				if (jtEntries.getModel() instanceof ObjectUnionTableModel)
					return list.getList(sortIndex).get(row);
				else
					{
						SourceTrace st = null;
						if (row < pathwaysCount)
		...
		public void setSelectedRow(int row)
			{
				...
				if (currentRow != row)
					jtEntries.setRowSelectionInterval(row, row);
			}
						...
						setPreferredSize(new Dimension(400, 300));
							{
								...
								jspTable.setPreferredSize(new java.awt.Dimension(400, 300));
									{
										TableModel jtEntriesModel = new ObjectUnionTableModel(false);// new
										// LiveEntriesTableModel();
				...
				if (merge != null)
					{
						...
						TableModel jtEntriesModel = new ObjectUnionTableModel(false);
						sorter = new TableRowSorter<TableModel>(jtEntriesModel);
						sorter.addRowSorterListener(this);
						jtEntries.setRowSorter(sorter);
						jtEntries.setModel(jtEntriesModel);
		...
		public void loadDataSourceMerge(DataSourceMerge merge, boolean sourceA, int listMask, boolean readonly)
			{
				...
				this.list = merge.getThose(listMask);
				TableModel jtEntriesModel = new ObjectUnionTableModel(readonly);
				sorter = new TableRowSorter<TableModel>(jtEntriesModel);
				sorter.addRowSorterListener(this);
				jtEntries.setRowSorter(sorter);
				jtEntries.setModel(jtEntriesModel);
		...
		public final void setViewOptions(int viewOptions)
			{
				if (this.viewOptions == viewOptions)
					return;
				...
				if ((this.viewOptions & DataSourceUI.VO_WHICH) == (viewOptions & DataSourceUI.VO_WHICH))
					return;
