...
%% INITIAL
clc
disp('Running FHM_E2_VTG_mRNA_run...')
clear all 
format shorte
global param
...
timexx = [];%initialize -  holds simulated times
simdatax = [];%initialize - hold simulated data
simdataxx = [];%initialize - hold simulated data
...
%Now run for FAD_dose = 0 to allow fold change simulation for mRNA data
FHM_initial; %initializes parameter values
param.FAD_conc = 0; %control
...
for i = 1:2:length(event_list)
    ...
    %Hold onto simulated data as we step through the event list
    timexx = [timexx; time]; %Save times from previous run
    simdataxx = [simdataxx; simdata]; %Save results from prevous run
...
subplot(1,3,1), plot((timex(xx):timex(end))/24,simdatax(xx:end,11))
hold on
subplot(1,3,1), plot(E2_data(:,1)/24, E2_data(:,2),'kx')
hold off
xlabel('Days')
ylabel('Venous E2 (uM)')
axis([-2, 35, 0, 0.1])
...
%plot time in days rather than hours
subplot(1,3,2), plot((timex(xx):timex(end))/24,simdatax(xx:end,14))
hold on
subplot(1,3,2), plot(VTG_data(:,1)/24, VTG_data(:,2),'ko')
hold off
xlabel('Days')
...
%Plot ovarian CYP19A mRNA fold change simulations against data
%Ovarian CYP19A mRNA is state 14
%plot time in days rather than hours
subplot(1,3,3), plot((timex(xx):timex(end))/24,mRNA_fold_change(xx:end))
hold on
subplot(1,3,3), plot(mRNA_data(:,1)/24, mRNA_data(:,2),'b*')
hold off
xlabel('Days')
ylabel('Ovarian CYP19A mRNA fold change')
axis([-2, 35, 0, 8])
title(['CYP19A mRNA, Fadrozole ' num2str(FAD_dose) ' ug/L'])
