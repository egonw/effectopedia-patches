...
public class DTsGroupedDualYDispalySettings extends DTsGroupedDispalySettings
	{
		
		protected DTsGroupedDualYDispalySettings(DataTemplates templates, DataTemplatesTableModel tableModel)
			{
		...
		@Override
		public DTsGroupedDualYDispalySettings updateChart(ChartPanel cpChart)
			{
								...
								for (int grp = 0; grp < series.length; grp++)
									{
										...
										if (templateIndex < secondary)
											...
											{
												ChartDataSeries chartSeries = new ChartDataSeries(series[grp], cpChart.primaryXAxis, cpChart.secondaryYAxis, template.getMarkerType(), template.getMarkerOutlineColor());
												chartSeries.getMarker().setSize(template.getMarkerSize());
												chartSeries.getMarker().fillColor = getColorFromTemplate(template, grp, series.length-1);
